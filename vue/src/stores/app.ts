...
export const useAppStore = defineStore('app', () => {
  ...
  const platformModules = {
    binance: {
      // ========== Binance 特有功能 ==========
      // 模拟积分
      openSimulation: computed(() => binanceOpenSimulation.value),
      // 积分显示模式
      scoreDisplayMode: computed(() => binanceScoreDisplayMode.value),
      // 切换模拟积分按钮显示状态
      toggleSimulation: () => {
      ...
      config: computed(() => {
        const cfg = currentUser.value?.binance?.config
        if (cfg) {
      ...
      updateUserConfigAction: async ({ configKey, configValue, name, action }: { configKey: string, configValue: any, name: string, action: string }) => {
        try {
          ...
          const oldConfig = JSON.parse(
            JSON.stringify((currentUser.value.binance.config as any)[configKey]),
          )
      ...
      updateUserConfigsAction: async (configs: Record<string, any>) => {
        try {
          ...
          // 本地更新
          currentUser.value.binance.config = {
            ...currentUser.value.binance.config,
            ...configs,
          }
