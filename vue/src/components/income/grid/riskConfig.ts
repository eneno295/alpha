import type { RiskConfig, SimulationPresetGroup, Tier } from './types'

export const riskConfig: RiskConfig = {
  okx: {
    label: 'OKX',
    reserveRate: 5.76,
    symbols: {
      ETH: {
        makerFeeRate: 0.02,
        takerFeeRate: 0.05,
        tiers: [
          { cap: 5000, rate: 0.4 },
          { cap: 10000, rate: 0.5 },
          { cap: 25000, rate: 0.75 },
          { cap: 50000, rate: 1.25 },
          { cap: 75000, rate: 1.75 },
          { cap: 100000, rate: 2.25 },
          { cap: 125000, rate: 2.75 },
          { cap: 150000, rate: 3.25 },
          { cap: 175000, rate: 3.75 },
          { cap: Infinity, rate: 4.25 },
        ],
      },
      BTC: {
        makerFeeRate: 0.02,
        takerFeeRate: 0.05,
        tiers: [
          { cap: 1000, rate: 0.4 },
          { cap: 5000, rate: 0.5 },
          { cap: 20000, rate: 0.75 },
          { cap: 40000, rate: 1.25 },
          { cap: 60000, rate: 1.75 },
          { cap: 80000, rate: 2.25 },
          { cap: 100000, rate: 2.75 },
          { cap: 120000, rate: 3.25 },
          { cap: 140000, rate: 3.75 },
          { cap: Infinity, rate: 4.25 },
        ],
      },
    },
  },
  binance: {
    label: 'Binance',
    reserveRate: 6,
    symbols: {
      ETH: {
        makerFeeRate: 0.02,
        takerFeeRate: 0.05,
        tiers: [
          { cap: 5000, rate: 0.4 },
          { cap: 10000, rate: 0.5 },
          { cap: 25000, rate: 0.75 },
          { cap: 50000, rate: 1.25 },
          { cap: 75000, rate: 1.75 },
          { cap: 100000, rate: 2.25 },
          { cap: 125000, rate: 2.75 },
          { cap: 150000, rate: 3.25 },
          { cap: 175000, rate: 3.75 },
          { cap: Infinity, rate: 4.25 },
        ],
      },
    },
  },
  gate: {
    label: 'Gate',
    reserveRate: 2.4,
    symbols: {
      ETH: {
        makerFeeRate: 0.015,
        takerFeeRate: 0.046,
        tiers: [
          { cap: 100000, rate: 0.3 },
          { cap: 300000, rate: 0.35 },
          { cap: 1000000, rate: 0.4 },
          { cap: 1500000, rate: 0.45 },
          { cap: 2000000, rate: 0.5 },
          { cap: 3000000, rate: 0.7 },
          { cap: 5000000, rate: 0.75 },
          { cap: 7000000, rate: 0.8 },
          { cap: 12000000, rate: 1.0 },
          { cap: 20000000, rate: 1.2 },
          { cap: 30000000, rate: 1.6 },
          { cap: 50000000, rate: 2.0 },
          { cap: 70000000, rate: 2.5 },
          { cap: 100000000, rate: 3.0 },
          { cap: 200000000, rate: 5.0 },
          { cap: 300000000, rate: 7.0 },
          { cap: 500000000, rate: 10.0 },
          { cap: 700000000, rate: 12.0 },
          { cap: 1000000000, rate: 25.0 },
          { cap: 1500000000, rate: 50.0 },
          { cap: Infinity, rate: 50.0 },
        ],
      },
      BTC: {
        makerFeeRate: 0.015,
        takerFeeRate: 0.046,
        tiers: [
          { cap: 200000, rate: 0.3 },
          { cap: 500000, rate: 0.35 },
          { cap: 1000000, rate: 0.4 },
          { cap: 1500000, rate: 0.45 },
          { cap: 2000000, rate: 0.5 },
          { cap: 3000000, rate: 0.7 },
          { cap: 5000000, rate: 0.75 },
          { cap: 7000000, rate: 0.8 },
          { cap: 12000000, rate: 1.0 },
          { cap: 20000000, rate: 1.2 },
          { cap: 30000000, rate: 1.6 },
          { cap: 50000000, rate: 2.0 },
          { cap: 70000000, rate: 2.5 },
          { cap: 100000000, rate: 3.0 },
          { cap: 200000000, rate: 5.0 },
          { cap: 300000000, rate: 7.0 },
          { cap: 500000000, rate: 10.0 },
          { cap: 700000000, rate: 12.0 },
          { cap: 1000000000, rate: 25.0 },
          { cap: 1500000000, rate: 50.0 },
          { cap: Infinity, rate: 50.0 },
        ],
      },
    },
  },
}

export function tiersToText(tiers: Tier[]): string {
  return tiers.map((tier) => `${Number.isFinite(tier.cap) ? tier.cap : 'INF'}:${tier.rate.toFixed(2)}`).join(',')
}

export const simulationPresets: SimulationPresetGroup[] = [
  {
    group: 'BTC',
    presets: [
      {
        key: 'btc1000',
        label: 'BTC 1000 / 70 / 10x Gate',
        platformKey: 'gate',
        symbolKey: 'BTC',
        values: {
          gridMode: 'arithmetic',
          investAmount: 1000,
          multiplier: 10,
          grids: 70,
          rangeLow: 45888,
          rangeHigh: 80888,
          estimatePrice: 40000,
          estimateGridDiff: 5000,
          dailyFilledGrids: 10,
        },
      },
      {
        key: 'btc5000',
        label: 'BTC 5000 / 70 / 10x',
        platformKey: 'okx',
        symbolKey: 'BTC',
        values: {
          gridMode: 'arithmetic',
          investAmount: 5000,
          multiplier: 10,
          grids: 70,
          rangeLow: 40000,
          rangeHigh: 80000,
          estimatePrice: 40000,
          estimateGridDiff: 5000,
          dailyFilledGrids: 10,
        },
      },
    ],
  },
  {
    group: 'ETH',
    presets: [
      {
        key: 'eth2000',
        label: 'ETH 2000 / 70 / 10x',
        values: {
          gridMode: 'arithmetic',
          investAmount: 2000,
          multiplier: 10,
          grids: 70,
          rangeLow: 1026,
          rangeHigh: 2426,
          estimatePrice: 100,
          estimateGridDiff: 100,
          dailyFilledGrids: 8,
        },
      },
      {
        key: 'eth2500',
        label: 'ETH 2500 / 70 / 10x',
        values: {
          gridMode: 'arithmetic',
          investAmount: 2500,
          multiplier: 10,
          grids: 70,
          rangeLow: 1026,
          rangeHigh: 2426,
          estimatePrice: 100,
          estimateGridDiff: 100,
          dailyFilledGrids: 8,
        },
      },
      {
        key: 'eth3000',
        label: 'ETH 3000 / 70 / 10x',
        values: {
          gridMode: 'arithmetic',
          investAmount: 3000,
          multiplier: 10,
          grids: 70,
          rangeLow: 1026,
          rangeHigh: 2426,
          estimatePrice: 100,
          estimateGridDiff: 100,
          dailyFilledGrids: 8,
        },
      },
      {
        key: 'eth3500',
        label: 'ETH 3500 / 70 / 10x',
        values: {
          gridMode: 'arithmetic',
          investAmount: 3500,
          multiplier: 10,
          grids: 70,
          rangeLow: 1026,
          rangeHigh: 2426,
          estimatePrice: 100,
          estimateGridDiff: 100,
          dailyFilledGrids: 8,
        },
      },
      {
        key: 'eth4000',
        label: 'ETH 4000 / 70 / 10x',
        values: {
          gridMode: 'arithmetic',
          investAmount: 4000,
          multiplier: 10,
          grids: 70,
          rangeLow: 1026,
          rangeHigh: 2426,
          estimatePrice: 100,
          estimateGridDiff: 100,
          dailyFilledGrids: 8,
        },
      },
      {
        key: 'eth4500',
        label: 'ETH 4500 / 70 / 10x',
        values: {
          gridMode: 'arithmetic',
          investAmount: 4500,
          multiplier: 10,
          grids: 70,
          rangeLow: 1026,
          rangeHigh: 2426,
          estimatePrice: 100,
          estimateGridDiff: 100,
          dailyFilledGrids: 8,
        },
      },
      {
        key: 'eth5000',
        label: 'ETH 5000 / 70 / 10x',
        values: {
          gridMode: 'arithmetic',
          investAmount: 5000,
          multiplier: 10,
          grids: 70,
          rangeLow: 1026,
          rangeHigh: 2426,
          estimatePrice: 100,
          estimateGridDiff: 100,
          dailyFilledGrids: 8,
        },
      },
      {
        key: 'eth1500-30',
        label: 'ETH 1500 / 30 / 10x',
        values: {
          gridMode: 'arithmetic',
          investAmount: 1500,
          multiplier: 10,
          grids: 30,
          rangeLow: 1626,
          rangeHigh: 2226,
          estimatePrice: 100,
          estimateGridDiff: 100,
          dailyFilledGrids: 8,
        },
      },
      {
        key: 'eth2000-30',
        label: 'ETH 2000 / 30 / 10x',
        values: {
          gridMode: 'arithmetic',
          investAmount: 2000,
          multiplier: 10,
          grids: 30,
          rangeLow: 1626,
          rangeHigh: 2226,
          estimatePrice: 100,
          estimateGridDiff: 100,
          dailyFilledGrids: 8,
        },
      },
      {
        key: 'eth2500-30',
        label: 'ETH 2500 / 30 / 10x',
        values: {
          gridMode: 'arithmetic',
          investAmount: 2500,
          multiplier: 10,
          grids: 30,
          rangeLow: 1626,
          rangeHigh: 2226,
          estimatePrice: 100,
          estimateGridDiff: 100,
          dailyFilledGrids: 8,
        },
      },
      {
        key: 'eth20000-70',
        label: 'ETH 20000 / 70 / 1x',
        values: {
          gridMode: 'arithmetic',
          investAmount: 20000,
          multiplier: 1,
          grids: 70,
          rangeLow: 926,
          rangeHigh: 2326,
          estimatePrice: 100,
          estimateGridDiff: 100,
          dailyFilledGrids: 3.5,
        },
      },
    ],
  },
  {
    group: 'SOXL',
    presets: [
      {
        key: 'soxl5000-30',
        label: 'SOXL 5000 / 30 / 2x',
        values: {
          gridMode: 'arithmetic',
          investAmount: 5000,
          multiplier: 2,
          grids: 30,
          rangeLow: 150,
          rangeHigh: 200,
          estimatePrice: 1,
          estimateGridDiff: 10,
          dailyFilledGrids: 10,
        },
      },
    ],
  },
]
