...
export async function testApiConnection() {
  try {
    ...
    if (response.ok) {
      ...
      apiStatus.value.isConnected = true;
      apiStatus.value.lastError = null;
      return { success: true, data };
    } else {
      const errorData = await response.json();
      apiStatus.value.isConnected = false;
      apiStatus.value.lastError = errorData.msg || '未知错误';
      return { success: false, error: errorData.msg || '未知错误' };
    }
  } catch (error: any) {
    apiStatus.value.isConnected = false;
    apiStatus.value.lastError = error.message;
